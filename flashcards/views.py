...
def flashcards(request):
    ...
    context = {
        "topics": topic_query_set
    }  # Dictionary{} rendered for flashcards/flashcards.html
    ...
    if request.method == "POST":
        ...
        if form.is_valid():  # Check if form is valid
            ...
            return HttpResponseRedirect(
                reverse("flashcards:flashcards")
            )  # Redirect to the flashcards view
...
def view_card_set(request, card_set_id):
    ...
    card_set_object = get_object_or_404(Card_Set, id=card_set_id)
    card_list = (
        card_set_object.card_set.all()
    )  # Returns all cards in respective card set
